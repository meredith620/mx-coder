...
- 如果 session 还没有 thread id，则 attach 时启动新线程，等待 Codex 产生 `thread.started`，然后回填 session 的 thread id。
- `codex resume` 的真实参数支持 `--last`、`--all`、`--include-non-interactive`，并保留 `--cd`、`--add-dir`、`--model`、`--sandbox`、`--ask-for-approval` 等常用选项。
- `codex` 不带子命令时会进入交互式 CLI，参数会转发给 interactive CLI；`codex exec` 是非交互模式。
...
因此，`mx-coder` 要做 IM 实时审批，不能依赖一个不存在的 Codex CLI flag；需要把审批交互放在 mx-coder 的 bridge / worker 层上，再把 allow / deny 结果喂回 Codex 执行链。
