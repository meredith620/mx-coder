-# mm-coder (Multi-modal Coder) — 需求与设计规格
...
- **attach 退出后立即 pre-warm**：`mm-coder attach` 退出后，daemon 立即为该 session 启动或恢复 IM worker，使后续 IM 消息可直接复用现有上下文
...
- **context window 自管理**：Claude Code 内部自动压缩/摘要历史，mm-coder 不额外做 context 管理；常驻 worker 直接复用同一 Claude 会话上下文
...
终端 attach:
  mm-coder attach bug-fix
    ...
    → daemon 立即 pre-warm IM worker（spawn claude -p --input-format stream-json --resume <id> --permission-prompt-tool "node /tmp/mm-coder-mcp-bridge-<id>.js"）
...
- 支持两种 JSON 结构（二选一）：
  - 顶层直配：`{ "url": "...", "token": "...", "channelId": "...", "spaceStrategy": "thread" }`
  - 命名分组：`{ "mattermost": { "url": "...", "token": "...", "channelId": "...", "spaceStrategy": "thread" } }`
...
- `claude -p` 常驻 worker 崩溃：进入 `recovering`，执行退避重启（1s/3s/10s）；超过次数阈值进入 `error`
- 单条消息超时：当前消息标记 `failed`，worker 可被重置并重建；session 回到 `idle` 或 `recovering`，IM 回复“本条任务超时，请重试”（默认 `messageTimeoutSeconds=600`）
...
| `/status` | 显示全局或当前 session 状态；包含当前 Mattermost 默认 `spaceStrategy` 与当前 session 实际绑定空间类型 | `isTopLevel ? '' : 当前会话空间` |
...
| 命令 | 行为 | 回复目标 |
|------|------|---------|
| `/remove`、`/attach`、`/create` 及任何其他 `/xxx` | 返回"该命令不支持在 IM 中使用，请使用 mm-coder CLI" | 当前会话空间 |
