...
终端 attach:
  mm-coder attach bug-fix
    ...
    → 若 session 处于 im_processing：
       ...
       → daemon 在 im_processing 完成并停止 IM worker 后，向等待中的 CLI 发送 `{type:"event",event:"attach_ready",data:{name}}`
...
Channel 主消息流 → 信息/导航命令（/help, /list, /status, /open）+ 顶层普通文本
  /list   → 列出所有 session 及其绑定 thread（当前实现展示 threadId）
  /open   → 若目标 session 未绑定 thread，则创建新的独立 thread 并绑定；若已绑定，则在目标 thread 发定位消息
  普通文本 → 以当前 root post 作为 thread 起点，自动创建/命中一个 thread 级 IM 会话（`im-*`）

Thread（每个 session 一个） → 会话交互
  /open   → 与顶层相同：未绑定则创建新 thread，已绑定则发定位消息
...
interface Session {
  ...
  spawnGeneration: number;         // 每次尝试启动 IM worker +1，防止 pre-warm/lazy spawn 双启动
...
- `mm-coder attach` 退出上报 ⇄ `{command:"attach_exit", args:{name, reason}}`
...
- `mm-coder acl get <name>` ⇄ `{command:"acl_get", args:{name}}`
- `mm-coder acl grant <name> --user <id> --role <owner|operator|approver>` ⇄ `{command:"acl_grant", args:{name, userId, role}}`
- `mm-coder acl revoke <name> --user <id> --role <owner|operator|approver>` ⇄ `{command:"acl_revoke", args:{name, userId, role}}`
...
- Claude Code IM 审批：**仅采用 PermissionRequest（MCP）机制**
  ...
  - IM worker 通过 `--permission-prompt-tool "node /tmp/mm-coder-mcp-bridge-<sessionId>.js"` 连接 daemon MCP server
...
#### A. 信息命令（isTopLevel 或 thread 均可）

| 命令 | 行为 | 回复目标 |
|------|------|---------|
| `/help` | 显示帮助文本 | `isTopLevel ? '' : msg.threadId` |
| `/list` | 列出所有 session 及绑定 thread | `isTopLevel ? '' : msg.threadId` |
| `/status` | 显示全局或当前 session 状态 | `isTopLevel ? '' : msg.threadId` |

#### B. 导航命令（isTopLevel 或 thread 均可）

| 命令 | 行为 | 回复目标 |
|------|------|---------|
| `/open <name>` | 若目标 session 未绑定 thread，则创建新的独立 thread 并绑定；若已绑定，则当前上下文收确认 + 目标 thread 收定位消息 | 当前上下文 ack + 目标 binding.threadId（若需新建则先 `createLiveMessage`） |

#### C. 不支持的命令（仅 thread 中返回错误，不落入 CLI）

| 命令 | 行为 | 回复目标 |
|------|------|---------|
| `/remove`、`/attach`、`/create` 及任何其他 `/xxx` | 返回"该命令不支持在 IM 中使用，请使用 mm-coder CLI" | `msg.threadId` |
...
| 内容 | 行为 |
|------|------|
| 普通文本消息（非 `/` 开头） | 进入 Claude 处理流程，消息入队由 `IMMessageDispatcher` 串行处理；若当前 thread 尚未绑定任何 session，则自动创建一个 thread 级 IM 会话（`im-*`）并绑定到该 thread |
...
用户: /list
Bot:  ● bug-fix    (idle)      ~/myapp    → [thread=abc123]
      ● review-pr  (attached)  ~/other    → [thread=def456]  ← 终端使用中

用户: /open bug-fix
Bot:  已在会话 bug-fix 的 thread 中发送定位消息。

用户: /open demo   # demo 尚未绑定任何 thread
Bot:  已为会话 demo 创建独立 thread。已在该 thread 发送定位消息，可直接开始对话。

用户: 这个 root post 直接跟进一下
Bot:  [自动创建/命中当前 root post 对应的 thread 级 IM 会话，并进入 Claude 处理流程]


# ===== Thread 内会话（直接对话，无需前缀）=====
...
用户: /open bug-fix
Bot:  [在当前上下文发确认，目标 session thread 发定位消息]
...
> `runtimeStatus`（idle/attached/im_processing 等）与 `lifecycleStatus`（active/stale/archived）**独立演进**，互不阻塞。

**runtimeStatus**（见 §3.6 状态机）：描述会话当前运行态，随交互事件实时迁移。  
**lifecycleStatus**（见 §3.2）：描述会话整体健康阶段，迁移相对缓慢：

- `active` → `stale`：runtimeStatus 为 `idle` 持续超过 `sessionTimeoutMinutes`
- `stale` → `archived`：持续超过 `retention.sessionArchiveDays`（无论 runtimeStatus 如何）
