...
- [ ] **spaceStrategy 配置落地**：Mattermost 配置新增 `spaceStrategy=thread|channel`，默认 `thread`；修改配置后仅影响未来新建 session，既有 session 不迁移
  - 对应 SPEC §2.4 / §2.7
- [ ] **单次 override 能力**：CLI / TUI 的单次创建或 open 动作可 override 全局默认 `spaceStrategy`，但 override 不回写配置文件
  - 对应 phase3 规划与后续实现切片
- [ ] **`/status` / CLI status / TUI 展示当前模式**：全局与会话视图中展示当前 Mattermost 默认新建模式（thread / channel）以及当前 session 的实际绑定空间类型
  - 对应用户确认的产品要求
- [ ] **新 session 创建入口统一受策略控制**：包括 CLI 创建、TUI 创建、IM `/open` 与 IM 普通文本自动建 session；都必须使用当前 `spaceStrategy`
  - 对应 SPEC §2.4 / §7
- [ ] **channel 模式仅对 Mattermost 生效**：为未来 Discord 等插件保留扩展点，但当前不承诺跨 IM 通用 channel/thread 语义
  - 对应用户确认的范围约束
- [ ] **channel 模式清理优先 archive/解绑，而非默认硬删除**：未来 remove/archive 语义需显式区分 thread 与 channel 两类空间，对 channel 默认采取 archive/解绑优先策略
  - 对应用户确认的产品约束
